...
ID3D10Effect* fx::GenGBufferFX     = 0;
ID3D10Effect* fx::LightingFX       = 0;
ID3D10Effect* fx::ShadingFX        = 0;
...
void fx::InitAll(ID3D10Device* device)
{
    ...
    GenGBufferFX     = CreateFX(device, L"gengbuffer.fx");
    LightingFX       = CreateFX(device, L"lighting.fx");
    ShadingFX        = CreateFX(device, L"shading.fx");
}
...
void fx::DestroyAll()
{
    ...
    ReleaseCOM(GenGBufferFX);
    ReleaseCOM(LightingFX);
    ReleaseCOM(ShadingFX);
}